...
Results saved to `results/final_unified_benchmark.csv` and `results/final_comparison_table.csv`.
...
- **Best classical model:** SVM (RBF kernel) — higher accuracy than Random Forest with a compact support-vector representation.
- **Best deep learning model:** Model B (Mini-SqueezeNet) — highest accuracy across all three quantization variants.
- **Recommended deployment model:** **Model B PTQ** — within ~0.3% of QAT accuracy at a fraction of the optimization cost, with lower inference latency. Suitable for real-time, resource-constrained deployment.
- Confusion matrix analysis shows that QAT significantly reduces misclassification between the phonetically similar pair *tree* / *three*, demonstrating that fine-tuning under simulated quantization noise improves acoustic feature preservation.
